...
namespace mongo {
        ...
        Status WiredTigerSnapshotManager::prepareForSnapshot(OperationContext* txn) {
            // Make sure there are no in-flight capped inserts while we create our snapshot.
            Lock::ResourceLock cappedInsertLock(txn->lockState(),
                                                WiredTigerRecordStore::kCappedInsertResource,
                                                MODE_X);

            WiredTigerRecoveryUnit::get(txn)->prepareForSnapshot(txn);
            return Status::OK();
        }

        Status WiredTigerSnapshotManager::createSnapshot(OperationContext* txn,
